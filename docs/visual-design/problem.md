...
- **Operator visual input**: a Figma file, a stack of screenshots, a hand-drawn sketch they photographed, a competitor
  product they're pointing at, a brand style guide PDF.
- **Visual judgment**: does this implementation match the mock? Are the components consistent across screens? Is this
  accessible? Does it look right?
...
**Operator drops reference files in `.jig/design/`** (Figma exports, screenshots, sketches, PDFs of style guides). Suite
briefs gain a `visual_references` field listing the relevant files. When the orchestrator dispatches a UI ticket, it
injects the relevant references into the dev agent's context using a vision-capable model. No new role, no formal visual
contracts, no automated visual review — just operator-authored references and agent vision consumption.

That works for a single operator who already has design assets and wants the agent to match them. It's a genuine v1
floor: most UI projects could ship usefully under just this much.

What it does NOT do: catch divergence between agents (each implements differently within the bounds of "looks roughly
like the mock"), enforce a component library (each agent invents Button differently), provide visual review (operator
has to eyeball every screen), or scale to projects without pre-existing mocks.
...
- **Operator workflow**: the TUI is text-only. Showing the operator a visual mock during PO discovery, or a screenshot
  ...
  browser-based companion (similar to the visual companion in some claude skills). Real but punt-able for v1.
...
- **Generating mocks from text descriptions.** Some agents can do this; it's not jig's job. Operator brings visual
  input; jig consumes it.
...
- **Replacing design tools.** Figma, Sketch, Penpot, Excalidraw all stay in operator's hands. Jig reads exports, not
  source files.
- **Animation / transition / motion design** as a first-class discipline. May come back; not v1.
...
- For a UI project with operator-provided mocks: dev agents implement screens that match those mocks at the level of
  obvious divergence (layout, hierarchy, primary colors). Operator can quickly approve or flag.
- For a backend-only project: zero ceremony — no `.jig/design/`, no `visual_references` field used, no vision models
  invoked.
- Operator can update a mock, mark it as the new reference, and trigger re-implementation of affected screens.
- Visual divergence between two screens that share UI primitives (Button, Form) is detectable — at minimum the reviewer
  surfaces "these two screens use Button differently." (Likely deferred behind v1.)
...
- [ ] **Where in the workflow sequence?** Parallel to SA (both consume PO discovery, both produce contracts)? After SA
  ...
  artifacts.
- [ ] **What's the visual contract format?** Free-form file references in v1 is fine. For the eventual component
...
- [ ] **Source-of-truth and re-import.** If operator updates an external design tool but doesn't re-export, jig works
  ...
  the latter for v1; integration-via-API is per-tool and deferred.
