...
- `async def _watch_completion(queue: asyncio.Queue[dict]) -> tuple[dict, str | None]` —
  consumes frames from queue; on `kind == "project_complete"` records event data; on
  `kind == "analysis_complete"` captures `data["out_dir"]`; returns when both received or
  `analysis_complete` times out (60s after `project_complete`).
- `async def _watch_stall(queue: asyncio.Queue[dict], detector: StallDetector) -> StallVerdict`
  — runs two concurrent sub-tasks: a consumer that calls `detector.observe(frame)` for each frame
...
- `async def run_eval(project_id, *, label, keep, timeout_minutes, jig_repo) -> RunResult`:
  ...
  4. Starts `subprocess.Popen(["jig", "start", "--no-docker", "--path", str(temp_dir),
     "--ws-port", str(port)], stdout=DEVNULL, stderr=DEVNULL)`. Waits for addr file (15s timeout).
  5. Connects WS, creates `completion_q` and `stall_q`, starts a single `_dispatch` task that
     reads all WS frames and fans them into both queues; races `_watch_completion(completion_q)`,
     `_watch_stall(stall_q, detector)`, and `asyncio.sleep(timeout_seconds)` via
