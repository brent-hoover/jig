...
- **Role name typos** — if an agent passes `roles=["devi"]`, the record is silently written
  under a role that never loads it. Mitigation: tool description lists valid role names;
  no validation added (agents are trusted callers).
