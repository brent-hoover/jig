...
def _build_ticket_nodes(
    ...
    for api in touches.get("exposed_apis", []):
        parts = api.split(":", 1)
        if len(parts) == 2:
            edges.append(Edge(src=t_nid, dst=f"exposed_api:{parts[0]}:{parts[1]}", kind="touches"))
    for ev in touches.get("emitted_events", []):
        parts = ev.split(":", 1)
        if len(parts) == 2:
            edges.append(Edge(src=t_nid, dst=f"emitted_event:{parts[0]}:{parts[1]}", kind="touches"))
    ...
    for bc in touches.get("behavioral_contracts", []):
        parts = bc.split(":", 1)
        if len(parts) == 2:
            edges.append(Edge(src=t_nid, dst=f"behavioral_contract:{parts[0]}:{parts[1]}", kind="touches"))
    for dc in touches.get("data_contracts", []):
        parts = dc.split(":", 1)
        if len(parts) == 2:
            edges.append(Edge(src=t_nid, dst=f"data_contract:{parts[0]}:{parts[1]}", kind="touches"))
...
def ticket_impact(
    ...
    *,
    ...
    modules_in_walk = {
        ...
        if nid in node_map and node_map[nid].kind == "module"
    }
