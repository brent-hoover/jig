...
def _catalog(
    *,
    ...
    return CheckCatalog.model_validate({k: v.model_dump() for k, v in payload.items()})
...
class TestFailingGate:
    ...
    async def test_timeout_and_error_also_block(self, tmp_path: Path) -> None:
        results, threads = await _stores(tmp_path)
        await results.post(_result(check_name="unit", verdict="timeout", offset_s=0))
        await results.post(_result(check_name="lint", verdict="error", offset_s=10))
        verdict = await evaluate_handoff_gate(
...
class TestWarningSeverity:
    async def test_warning_fail_does_not_block(self, tmp_path: Path) -> None:
        results, threads = await _stores(tmp_path)
        await results.post(
            _result(
                check_name="lint",
        ...
        entries = await threads.for_ticket("tkt-1")
        assert not any(
            isinstance(e, SystemEvent) and e.event_type == "check_failure"
            for e in entries
        )
...
class TestMissingResults:
    ...
    async def test_mixed_missing_and_pass(self, tmp_path: Path) -> None:
        results, threads = await _stores(tmp_path)
...
class TestLatestRunWins:
    async def test_latest_pass_clears_earlier_fail(self, tmp_path: Path) -> None:
        results, threads = await _stores(tmp_path)
        await results.post(_result(check_name="unit", verdict="fail", offset_s=0))
        await results.post(_result(check_name="unit", verdict="pass", offset_s=10))
        verdict = await evaluate_handoff_gate(
            catalog=_catalog(required=["unit"]),
            results=results,
            threads=threads,
...
class TestSeverityRescoredFromCatalog:
    async def test_record_severity_stale_but_catalog_wins(self, tmp_path: Path) -> None:
...
class TestWaiver:
    async def test_waived_check_failure_clears_gate(self, tmp_path: Path) -> None:
        ...
        results, threads = await _stores(tmp_path)
        sha = "deadbeef" * 5
        result = _result(check_name="unit", verdict="fail").model_copy(
    ...
    async def test_waiver_does_not_carry_to_new_commit(self, tmp_path: Path) -> None:
...
class TestUnknownCheck:
    async def test_unknown_required_name_raises(self, tmp_path: Path) -> None:
        results, threads = await _stores(tmp_path)
