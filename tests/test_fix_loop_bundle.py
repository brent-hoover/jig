...
async def test_notables_never_in_bundle(tmp_path: Path) -> None:
    """Binary severity: notables never appear in fix-loop bundles, in any
    cycle, regardless of ack state — they become proposed issues instead."""
    notable = _comment(file="src/main.py", severity=Severity.NOTABLE, cycle=0)
    blocking = _comment(file="src/main.py", severity=Severity.IMPORTANT, cycle=1)
    out = await build_fix_loop_bundle(
        workflow=_wf(),
        blocked_phase_idx=3,
        target_phase_idx=2,
        all_comments=[notable, blocking],
        all_acks=[],
        worktree_path=tmp_path,
    )
    severities = [f["severity"] for f in out["findings"]]
    assert "notable" not in severities
    assert severities  # the blocking finding still routes
