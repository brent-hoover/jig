...
async def test_resume_arch_finalize_handoff_routes_to_already_done(wired):
    """sa_mvp exits via arch_finalize which posts Handoff(phase=pm) without a
    sa_propose_scaffold Note. classify_resume must return ALREADY_DONE so the
    dispatch loop stops respawning SA."""
    await wired["tickets"].create(
        Ticket(id="brief", work_type=WorkType.BRIEF, title="b", created_by="cli")
    )
    await wired["threads"].post(
    ...
    await wired["threads"].post(
        Handoff(ticket_id="architecture", author="sa_mvp", phase="pm", summary="done")
    )
    _seed_profile(wired["path"])
    assert (
        await classify_resume(
...
async def test_resume_sa_propose_scaffold_takes_precedence_over_arch_finalize(wired):
    ...
    await wired["threads"].post(
        Handoff(ticket_id="architecture", author="sa", phase="pm", summary="done")
    )
    _seed_profile(wired["path"])
    assert (
        await classify_resume(
