...
@pytest.mark.asyncio
async def test_phase_end_fires_with_failed_outcome_when_agent_raises(
    tmp_path: Path, monkeypatch
) -> None:
    """If ``run_agent`` raises before ``result`` is bound, the ``finally``
    block must still emit ``phase_end`` with ``outcome="failed"`` (exercising
    the ``result = None`` sentinel)."""
    workflow = WorkflowConfig(
        name="default",
        phases=[PhaseConfig(name="spec", role="spec")],
    )
    ...
    try:
        ...
        async def done() -> bool:
            t = await orch.tickets.get(tid)
            return t is not None and t.status == TicketStatus.FAILED
