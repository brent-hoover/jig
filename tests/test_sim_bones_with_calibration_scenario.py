...
@pytest.mark.asyncio
async def test_bones_with_calibration_passes_in_mock_mode(tmp_path: Path):
    """Calibration scenario runs end-to-end."""
    _seed_repo(tmp_path)
...
@pytest.mark.asyncio
async def test_bones_with_calibration_persists_jsonl(tmp_path: Path):
    """All five synthetic samples land in the calibration JSONL."""
    _seed_repo(tmp_path)
    scenario = load_scenario(SCENARIO_PATH)
    report = await Driver().run(scenario, project_root=tmp_path)
    assert report.passed, report.failure_summary()
    cal = tmp_path / ".jig" / "plan" / "calibration.jsonl"
    assert cal.is_file()
    body = cal.read_text()
    for i in range(1, 6):
        assert f"cal-warm-{i}" in body
