...
async def test_concierge_rejects_no_args():
    ...
    assert "query" in result["error"].lower() or "orchestrator" in result["error"].lower()


@pytest.mark.asyncio
async def test_concierge_rejects_no_orch():
    handler = get_handler("concierge")
    result = await handler(args=["what is jig?"], orch=None, project_path=None, emitter=None)
    assert result["ok"] is False
